...
import { useDeletePartnerDedicationByIdMutation } from "@/redux/features/partner-dedication/partner-dedication.api";
...
const DeletePartnerDedicationById = ({
  partnerDedicationId,
  partnerDedicationName,
}: {
  partnerDedicationId: string;
  partnerDedicationName: string;
}) => {
  const [isOpen, setIsOpen] = useState(false);
  const [inputValue, setInputValue] = useState("");

  const [deletePartnerDedication, { isLoading }] = useDeletePartnerDedicationByIdMutation();

  const handleDelete = async () => {
    if (inputValue !== "DELETE") return;
    const res = await deletePartnerDedication(partnerDedicationId);
    ...
    toast.success("Partner dedication deleted successfully");
    setIsOpen(false);
  };
...
export default DeletePartnerDedicationById;
