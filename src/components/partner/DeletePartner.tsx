import { useDeletePartnerByIdMutation } from "@/redux/features/partners/partner.api";
...
const DeletePartner: React.FC<IProps> = ({ partner }) => {
  const [isOpen, setIsOpen] = useState(false);

  const [inputValue, setInputValue] = useState("");
  const canConfirm = inputValue.trim().toUpperCase() === "DELETE";

  const [deletePartner, { isLoading }] = useDeletePartnerByIdMutation();

  const handleDelete = async () => {
    if (!canConfirm || isLoading) return;
    const res = await deletePartner(partner._id);
  ...
  return (
    <>
      <button
        ...
        className="cursor-pointer rounded-[4px] bg-danger/10 p-2 text-danger"
      >
        <FaRegTrashAlt />
      </button>
      <DialogProvider state={isOpen} setState={setIsOpen} className="w-full max-w-[500px]">
        <div className="relative w-full transform overflow-hidden rounded-lg bg-white px-6 py-5 shadow-xl transition-all">
          ...
          <div className="mt-3 text-center">
            <h3 className="text-lg font-medium text-primary">Delete Partner</h3>
            <p className="mt-2 text-sm text-muted">
              Are you sure you want to delete{" "}
              <span className="rounded bg-danger/10 px-1 py-0.5 font-medium text-danger">
                “{partner?.partnerName}”
              </span>
              ? Deleting this partner will permanently remove it from the system and their nominees
